...
#        self.sp = open(aport,'rb+')
    ...
    def initChip(self):
        ...
        #time.sleep(0.5)
        self.enterCMD()
        self.sendCMD("ST,255")       #Set remote continuous config
        ...
        self.sendCMD("SQ,16")        #optimize for low latency data transfers
    ...
    def releaseChip(self):
        ...
        #self.sendCMD("S*,0800")     #Set GPIO11 low to set Reset
        #self.reset()
#        self.sendCMD("ST,")         #Set remote 60second config
        self.sp.write("S&,0800\r")   #Set GPIO3 as low to kill power on reset - note this will fail to get ACK if the logger turns off
        read_line = self.sp.readline()
        mdebug(10, read_line)
        self.sp.flush()
        self.sp.write("S@,0800\r")   #Set GPIO3 as input to kill power on reset
        read_line = self.sp.readline()
        mdebug(10, read_line)
        self.sp.flush()
#        self.exitCMD()
    ...
    def cmdWriteMemory(self, addr, data):
        ...
        if self.cmdGeneric(0x31):
            ...
            for c in data:
                crc = crc ^ c
                dataString+=chr(c)
            self.sp.write(dataString+chr(crc))    
            self.sp.flush()
...
#            conf['fname'] = a
    ...
    try:
        ...
        print "Can't connect to port, ensure device is turned on and connected"
        sys.exit(4)
...
#            file(conf['fname'], 'wb').write(rdata)
        ...
        cmd.sp.close()              #leaves the bluetooth rfcomm free for other connections - if the logger hasnt turned itself off
